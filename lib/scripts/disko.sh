...
nix --extra-experimental-features nix-command --extra-experimental-features flakes run github:nix-community/disko -- --mode disko "$partition_layout"
...
sed -i "/imports\ =/cimports\ = [\ $layout_filepath\ ]++" "$FLAKE_ROOT"/hosts/{k-on,minimal}/hardware-configuration.nix
